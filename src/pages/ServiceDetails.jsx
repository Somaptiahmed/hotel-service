...
const ServiceDetails = () => {
  ...
  const [newReview, setNewReview] = useState({
    text: "",
    serviceTitle: "", // Will be dynamically updated
    rating: 1, // Default rating
    userName: user?.displayName || "Anonymous", // Default to 'Anonymous' if no displayName
    userPhoto: user?.photoURL || "", // Default to empty if no photoURL
    postedDate: new Date().toISOString(),
  });
  ...
  const handleReviewSubmit = async (e) => {
    ...
    const reviewData = {
      ...
      userEmail: user?.email, // Access user email from AuthContext
    };
